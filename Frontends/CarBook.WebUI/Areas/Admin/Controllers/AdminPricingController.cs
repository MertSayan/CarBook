...
namespace CarBook.WebUI.Areas.Admin.Controllers
{
    ...
    public class AdminPricingController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7070/api/Pricings");
        ...
        public async Task<IActionResult> CreatePricing(CreatePricingDto createPricingDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7070/api/Pricings", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync("https://localhost:7070/api/Pricings?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminPricing", new { area = "Admin" });
            }
        ...
        public async Task<IActionResult> UpdatePricing(int id)
        {
            ...
            var responseMessage = await client.GetAsync($"https://localhost:7070/api/Pricings/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdatePricing(UpdatePricingDto updatePricingDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7070/api/Pricings/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminPricing", new { area = "Admin" });
            }
