...
namespace CarBook.WebUI.Areas.Admin.Controllers
{
    ...
    public class AdminLocationController : Controller
    {
        ...
        public async Task<IActionResult> CreateLocation(CreateLocationDto createLocationDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7070/api/Locations", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync("https://localhost:7070/api/Locations?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminLocation", new { area = "Admin" });
            }
        ...
        public async Task<IActionResult> UpdateLocation(int id)
        {
            ...
            var responseMessage = await client.GetAsync($"https://localhost:7070/api/Locations/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateLocation(UpdateLocationDto updateLocationDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7070/api/Locations/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminLocation", new { area = "Admin" });
            }
